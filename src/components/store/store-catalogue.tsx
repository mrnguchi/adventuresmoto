  ...
  return <div className="store-layout">
    ...
    <section className="store-results" aria-labelledby="store-title">
      ...
      {filtered.length > 0 ? <><div className="store-product-grid">{filtered.slice((currentPage - 1) * limit, currentPage * limit).map((product) => <article className="store-product-card" key={product.id}><Link className="store-product-link" href={`/products/${encodeURIComponent(product.slug)}`}>
        ...
        <div className="store-product-copy"><span className="store-product-brand">{product.brand}</span><h2>{product.name}</h2><p className="store-product-price">{money(product.price)} {product.compareAtPrice !== null && product.compareAtPrice > product.price && <del>{money(product.compareAtPrice)}</del>}</p><p className="store-product-stock"><span className={product.inStock ? "is-in-stock" : ""}>{product.inStock ? "In stock" : "Out of stock"}</span>{product.inStock && product.sizes.length > 0 && <span>: {product.sizes.join(" · ")}</span>}</p></div>
