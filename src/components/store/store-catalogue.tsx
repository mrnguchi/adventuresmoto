...
export function StoreCatalogue({ name, products, unavailable, showSizeFilter = false, searchResults = false }: { name: string; products: StoreProduct[]; unavailable: boolean; showSizeFilter?: boolean; searchResults?: boolean }) {
  ...
  return <div className="store-layout">
    ...
    <section className="store-results" aria-labelledby="store-title">
      <header className="store-heading"><p>{searchResults ? "Search results" : "Gear for the ride ahead"}</p><h1 id="store-title">{name}</h1></header>
      ...
      <div className="store-search-row"><input type="search" aria-label={searchResults ? "Filter search results by product name" : "Search products in this category"} placeholder={searchResults ? "Filter these results…" : "Search this collection…"} value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} />{(brands.length > 0 || sizes.length > 0 || stock.length > 0 || priceFiltered || search) && <button onClick={clear}>Clear filters ×</button>}</div>
      {filtered.length > 0 ? <><div className="store-product-grid">{filtered.slice((currentPage - 1) * limit, currentPage * limit).map((product) => <article className="store-product-card" key={product.id}><Link className="store-product-link" href={`/products/${encodeURIComponent(product.slug)}`}>
        ...
        <div className="store-product-copy"><h2>{product.name}</h2><p className="store-product-price">{product.priceFrom && <small>From </small>}{money(product.price)} {product.compareAtPrice !== null && product.compareAtPrice > product.price && <del>{money(product.compareAtPrice)}</del>}</p><p className="store-product-stock"><span className={product.inStock ? "is-in-stock" : ""}>{product.inStock ? "In stock" : "Out of stock"}</span>{product.inStock && product.sizes.length > 0 && <span>: {product.sizes.join(" · ")}</span>}</p></div>
